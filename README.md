...
## 1. Introduction
ngxway is a high-performance, containerized and lightweight nginx gateway based on openresty.

## 2. Features

### (1) High-performance

Based on Openresty's support for Lua modules, ngxway achieves an average high performance of ```10w QPS / 8c16G``` while implementing the Nginx gateway by writing Lua modules and tuning Linux parameters. You can check the detailed [performance report](https://wgrape.github.io/ngxway/html/benchmark.html).
...
Built on Docker, decoupled from the underlying configuration of Nginx, saving 99% of the construction process, one command can automatically build its own Nginx gateway.

<!-- image -->

### (3) Lightweight

Compared with the complex gateway implementation, the whole project is much lighter. You can change various configurations of the project at will, and even modify and add various Lua scripts to improve the comprehensive performance of ngxway under your business.
